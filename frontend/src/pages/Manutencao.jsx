...
import {
  collection, getDocs, setDoc, deleteDoc, addDoc,
...
export default function Manutencao() {
  ...
  const [formOS, setFormOS] = useState({ tipoServico: "", placa: "", motoristaId: "", obs: "" });
  ...
  async function carregarTudo() {
    ...
    try {
      ...
      const [snapM, snapV, snapMot, snapOS] = await Promise.all([
  ...
  async function salvarOS(e) {
    ...
    try {
      ...
      setOrdensServico(prev => [{ id: ref.id, ...payload }, ...prev]);
      setFormOS({ tipoServico: "", placa: "", motoristaId: "", obs: "" });
    } catch (e) {
            ...
            <form onSubmit={salvarOS} className="grid-form-2" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 12 }}>
              ...
              <label style={{ ...s.fieldLabel, gridColumn: "1 / -1" }}>
                ...
                <textarea
                  ...
                  placeholder="Descreva o serviço executado, peças trocadas, custos, observações..."
                />
              </label>
              ...
              <div style={{ gridColumn: "1 / -1", display: "flex", justifyContent: "flex-end", gap: 8 }}>
                <button
                  type="button"
                  style={s.cancelBtn}
                  onClick={() => { setFormOS({ tipoServico: "", placa: "", motoristaId: "", obs: "" }); setErroOS(""); }}
                >
                  Limpar
                </button>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: ".88rem" }}>
                <thead>
                  <tr style={{ background: "#f8fafc", borderBottom: "1px solid #e2e8f0" }}>
                    ...
                    {canDelete && <th style={thOS}></th>}
                  </tr>
                </thead>
                <tbody>
                  {ordensServico.length === 0 ? (
                    <tr><td colSpan={canDelete ? 7 : 6} style={{ padding: "2rem", textAlign: "center", color: "#94a3b8" }}>Nenhuma OS criada ainda</td></tr>
                    ...
                    <tr key={os.id} style={{ borderBottom: "1px solid #f1f5f9" }}>
                      ...
                      <td style={{ ...tdOS, maxWidth: 380, whiteSpace: "normal", color: "#475569" }}>{os.obs || "—"}</td>
                      {canDelete && (
                        <td style={tdOS}>
                          <button onClick={() => excluirOS(os)} style={{ background: "transparent", border: "none", color: "#dc2626", cursor: "pointer", fontSize: ".78rem", fontWeight: 600 }}>
                            Excluir
                          </button>
                        </td>
                      )}
